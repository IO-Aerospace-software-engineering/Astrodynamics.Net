    ...
    public class Launch : Entity
    {
        ...
        double GetInclination(in DateTime epoch)
        {
            return Vector3.VectorZ.Rotate(LaunchSite.Body.Frame.ToICRF(epoch).Orientation)
                .Angle(GetOrbitalParameters(epoch).ToFrame(Frame.Frame.ICRF).SpecificAngularMomentum());
        }
        ...
        public double GetNonInertialAscendingAzimuthLaunch(in DateTime epoch)
        {
            double vrotx =
                ...
                LaunchSite.GetEphemeris(Frame.Frame.ICRF, epoch).Velocity.Magnitude();
        ...
        private LaunchWindow[] SearchLaunchWindows(in Window searchWindow)
        {
            ...
            {
                bool status = LaunchSite.GetEphemeris(Frame.Frame.ICRF, date).Position *
                    GetOrbitalParameters(date).ToFrame(Frame.Frame.ICRF).SpecificAngularMomentum() > 0.0;
                while (System.Math.Abs(step.TotalSeconds) > 1.0)
                {
                    date += step;
                    if (status != LaunchSite.GetEphemeris(Frame.Frame.ICRF, date).Position * GetOrbitalParameters(date)
                            .ToFrame(Frame.Frame.ICRF).SpecificAngularMomentum() > 0.0)
                    {
                        step *= -0.5;
                        status = !status;
                    }
                ...
                bool isAscending = LaunchSite.GetEphemeris(Frame.Frame.ICRF, date).Position *
                    GetOrbitalParameters(date).ToFrame(Frame.Frame.ICRF).AscendingNodeVector() > 0.0;
                if (isAscending)
                {
